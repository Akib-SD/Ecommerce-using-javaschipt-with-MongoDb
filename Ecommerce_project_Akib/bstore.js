...
const port = 2030;
app.use(express.static(__dirname + "/broot"));
app.use(
  session({
    secret: "kit2kat",
    resave: false,
...
var carts = [];
var orders = [];
var uploadfolder = path.join(__dirname, "broot", "images");
/////////////////////////////
connect();
getBsCategories();
getBsProducts();
app.get("/", async (req, res) => {
  await getBsCategories();
  await getBsProducts();
...
app.get("/products", async (req, res) => {
  if (!products.length) await getBsProducts();
  if (!categories.length) await getBsCategories();
